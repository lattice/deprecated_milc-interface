    ...
    )
{
  ...
  static double d_loop_coeff[48];
  static float  f_loop_coeff[48];
  static int length[48];

  if((milc_loop_coeff[0] != d_loop_coeff[0]) ||
      (milc_loop_coeff[1] != d_loop_coeff[6]) ||
      (milc_loop_coeff[2] != d_loop_coeff[24]) ){

    setLoopCoeffs(milc_loop_coeff, d_loop_coeff);
    for(int i=0; i<48; ++i) 
      f_loop_coeff[i] = d_loop_coeff[i];
    for(int i=0; i<6; ++i) 
      length[i] = 3;
    for(int i=6; i<48; ++i) 
      length[i] = 5;
